...
{
  // Acrescentado por mim para visualizar cada fonte
  char *fontes[7] = {
    "GLUT_BITMAP_8_BY_13",
    "GLUT_BITMAP_9_BY_15",
  ...
  for (i = 0; i < 7; i++)
  {
    ...
    glRasterPos2f(-win, win-(win*0.08) - win*.2*i);


    // GFRD: exibe nome da fonte
    glTranslatef(0.25*win, 0, 0);
    char *s = fontes[i];
    while (*s)
        glutBitmapCharacter(constantes_fontes[i], *s++);
